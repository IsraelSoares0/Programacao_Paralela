...
\title{Estimativa Estocástica de $\pi$ com OpenMP: Acumulação por \texttt{critical} versus Vetor Compartilhado}
...
\textbf{\texttt{rand()} vs. Xorshift32.} Esse é o fator que mais pesa nos tempos. Para $n\_pontos=10^9$,
V1 e V2 (\texttt{rand()}) levam cerca de $4,1$s, contra apenas $0,76$--$0,79$s de V3 e V4 (Xorshift32) --
uma diferença de $\sim 5,3\times$. A razão é que \texttt{rand()} mantém um único estado \emph{global},
compartilhado por todas as threads, e o protege com sincronização interna a cada chamada. Como o laço
chama \texttt{rand()} duas vezes por iteração, esse estado é lido/escrito centenas de milhões de vezes,
e cada modificação obriga o protocolo de coerência de cache a invalidar e retransmitir a mesma linha de
cache entre os núcleos (um caso de \emph{compartilhamento verdadeiro}). Isso serializa boa parte do
trabalho que deveria ser paralelo. O Xorshift32, por outro lado, guarda sua semente como variável
totalmente privada (registrador/pilha de cada thread), sem estado compartilhado, e por isso escala quase
linearmente com o número de threads.
...
O gargalo de desempenho não está na acumulação dos resultados parciais, mas na geração dos números
aleatórios. Um gerador com estado global (\texttt{rand()}) precisa de sincronização a cada chamada, o que
gera tráfego intenso de coerência de cache entre os núcleos e serializa boa parte do programa. Um gerador
com estado privado por thread (Xorshift32, usado aqui no lugar de \texttt{rand\_r()}, indisponível no
Windows) elimina esse problema e escala quase linearmente. Já \texttt{critical} e vetor compartilhado se
mostraram equivalentes, pois ambos custam apenas uma operação por thread, e não uma por iteração --
inclusive o vetor, apesar de compartilhar linhas de cache entre threads, não sofre falso compartilhamento
relevante por ser acessado tão raramente. A lição geral é que o estado verdadeiramente compartilhado (aqui,
o gerador de números aleatórios) é o que determina a escalabilidade, muito mais do que pequenas seções
críticas ou vetores usados com moderação.
